...
int main()
{
    ...
    monoid m = bind_maybe(bind_maybe(getInt(), wrap(y, print(y))), wrap(x, print_sqrt(x))); // the monad operators is lazy 
    evaluation(m) //eval and certificate if a non-valid number was inserted, if not just calculate the sqrt.
    return 0;
}