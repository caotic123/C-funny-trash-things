...
Enviroment put_agents(Map map_n, int t, int q)
{
    ...
    map_n[sort.x][sort.y].agents
        = cons(map_n[sort.x][sort.y].agents, create_agent((pos){ sort.x, sort.y }, 0));
...
void print_map(Enviroment e)
{
    ...
    for (int i = 0; i <= e.len - 1; i++) {
        for (int i_ = 0; i_ <= e.len - 1; i_++) {
            ...
            sprintf(buffer, "%s%s",
                (len(e.map[i][i_].agents) == 0 ? ((k && e.map[i][i_].rew) ? "" : "-") : min_buffer),
                e.map[i][i_].rew ? kind_r : k ? "K" : "");
            printf("%s", buffer);
        }
        printf("\n");
    }
...
pos searchPoint(Enviroment e, pos* positions, Agent m)
{
    ...
    for (int i = 0; i <= e.agents_len + 3; i++) {
        v = ((m.position.x != positions[i].x) || (m.position.y != positions[i].y))
            && (m.H <= e.map[positions[i].x][positions[i].y].rew);
        if (m.king && e.map[positions[i].x][positions[i].y].rew) {
            pos_prox = positions[i];
            b = true;
        }
        else if (!m.king && ((!b && v) || (v
                                              && calc_pos_dis(m.position, positions[i])
                                                  < calc_pos_dis(m.position, pos_prox)))) {
            pos_prox = positions[i];
            b = true;
        }
    }
...
Enviroment move_agents_to(Enviroment e, Agent* m, pos position)
{
    ...
    {
        return (
            pos){ x + (x > x_ ? -1 : (x == x_) ? 0 : 1), y + (y > y_ ? -1 : (y == y_) ? 0 : 1) };
    }

    pos npos = d__(m->position.x, m->position.y, position.x, position.y);

    if (m->H >= e.map[m->position.x][m->position.y].rew && npos.x < e.len && npos.y < e.len
        && npos.x >= 0 && npos.y >= 0) {
        e.map[m->position.x][m->position.y].agents
            = filter(e.map[m->position.x][m->position.y].agents, filter_by_agent_pointer);
